...
class EigentestEngine:
    ...
    def run(
        ...
        *,
        ...
        exts = self._adapter.source_extensions()
        ...
        all_invariants = self._extract_invariants(all_functions)

        return EigentestResult(functions=all_functions, invariants=all_invariants)

    def _extract_invariants(self, functions: list[FunctionNode]) -> list[Invariant]:
        ...
        for file_funcs in by_file.values():
            all_invariants.extend(self._extract_with_transitive_purity(file_funcs))

        return all_invariants

    def _extract_with_transitive_purity(self, file_funcs: list[FunctionNode]) -> list[Invariant]:
        ...
        invariants: list[Invariant] = []
        for func in file_funcs:
            invariants.extend(self._adapter.extract_invariants(func, known_impure=frozenset(known_impure)))
        return invariants
